...
def generate_sidebar(conf, conf_api, hidden=True):
    ...
    def write_subproject(desc, project, link, subsection=None):
        if project != conf_api:
            ...
            lines.append('    {desc} <{link}>'.format(desc=desc, link=link))

    #
    # Specify the sidebar contents here
    #

    toctree('Deepcell Kiosk', hidden)
    # write('Home', 'index')
    ...
    toctree('Kiosk Redis Consumer', hidden)
    write_subproject('Custom Consumers', 'kiosk-redis-consumer', 'README')
