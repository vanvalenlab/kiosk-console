...
4. Apply for a `quota of at least 1 GPU <https://cloud.google.com/compute/quotas#gpus>`_ and at least `16 *In-use IP addresses* <https://cloud.google.com/compute/quotas#ip_addresses>`_ for the *Compute Engine API* of your region (by default ``us-west1``). Default quota limits are 1 GPU per region. To use more than 1 GPU, you must also request individual quota increases for each machine type and region. This may take some time, as Google will need to approve each of these requests.
