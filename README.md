...
2. In order to add accelerated hardware to the clusters you will launch, you will need to [upgrade](https://cloud.google.com/free/docs/gcp-free-tier#how-to-upgrade) your Google Cloud account and [apply](https://cloud.google.com/compute/quotas) for a quota for at least 1 GPU.<sup>[2](#footnote2)</sup> This may take some time, as Google will need to approve each request. The DeepCell Kiosk uses `nvidia-tesla-t4` GPUs for inference by default.
